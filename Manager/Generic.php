...
class Generic extends Manager
{
    ...
    public function render($prefix = '', $suffix = '') {
        ...
        return $this->postRender ( sprintf ( $this->getOutputWrapper (), $this->getAllVarNames (), ($prefix ? $prefix . "\n" : ''), $output, ($suffix ? $suffix . "\n" : '') ) );
    }
    
    public function postRender($contents) {
        if (! self::$obfuscate) {
            return $contents;
        }
    ...
    public function renderDependencies(Component $component) {
        ...
        foreach ( $component->getDependencies () as $dependency ) {
            if ($dependency->hasDependencies ()) {
                $dependencies .= $this->renderDependencies ( $dependency );
            }
    ...
    public function componentFromXType($xtype, $config) {
        return $this->componentFactory ( $xtype, $config, Component::JSON )->setOption('xtype', $xtype);
    }
}